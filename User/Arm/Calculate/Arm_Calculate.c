 ...
 * @LastEditTime: 2024-04-30 14:19:46
...
void CalculateParallelArm(double *motor_position, double x, double y, double z)
{
    ...
    motor_position_tmp[0] = atan2(x,y) / DEC ;  //云台
    motor_position_tmp[1] = ( M_PI_2 - atan2( sqrt(x*x+y*y),z ) - acos( (x*x+y*y+z*z+L1*L1-L2*L2) / (2*L1*sqrt(x*x+y*y+z*z)) ) ) / DEC ;  //下部pitch轴电机
    motor_position_tmp[2] = ( - M_PI_2 + atan2( z,sqrt(x*x+y*y) ) + acos( (x*x+y*y+z*z+L2*L2-L1*L1) / (2*L2*sqrt(x*x+y*y+z*z)) ) ) / DEC; ;  //上部pitch轴电机
